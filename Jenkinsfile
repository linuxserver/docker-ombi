  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -Ls -w %{url_effective} -o /dev/null 'https://ci.appveyor.com/api/projects/tidusjar/requestplex/artifacts/linux1.tar.gz?branch=feature/v4&pr=false' | awk -F / '{print $6}' | sed 's/-/./g' ''',
            returnStdout: true).trim()
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:arm32v6-${META_TAG} ${IMAGE}:arm32v6-v4-preview"
          ...
          sh "docker push ${IMAGE}:arm32v6-v4-preview"
          sh "docker push ${IMAGE}:arm64v8-v4-preview"
          sh "docker manifest push --purge ${IMAGE}:v4-preview || :"
          sh "docker manifest create ${IMAGE}:v4-preview ${IMAGE}:amd64-v4-preview ${IMAGE}:arm32v6-v4-preview ${IMAGE}:arm64v8-v4-preview"
          sh "docker manifest annotate ${IMAGE}:v4-preview ${IMAGE}:arm32v6-v4-preview --os linux --arch arm"
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}' to v4-preview",\
             "type": "commit",\
